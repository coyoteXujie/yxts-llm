...
8. 平安镇任务链到“英雄试炼”的阶段通关目标，接入“洛阳旧火/暗影书信/七派风声/暗影眼线/断令归卷/武林夜议”的主线开端，并加入七派各 3 段入门任务线。

这条竖切已包含背包、商店、任务、门派修炼、七派入门任务线、区域探索、带 NPC/任务标记的世界地图面板、存档读档和基础 NPC/地图/物品/UI 美术资源。后续重点转向更长的门派晋升剧情、战斗招式和 LLM 对话。
...
- `LocalAreaMap`：承接世界地图进入后的城镇/门派/野外局部地图和商铺内景，负责区域内商铺入口、可互动地标、每日资源点、相邻区域路牌、返回世界入口、局部 NPC 摆放、地貌差异化布局和内景碰撞。
...
- 已实装每日资源点：城池、镇子、门派、野外会按地貌生成药草、鱼篓、食盒、矿点、香案供钱等资源入口，同日重复搜寻只展示已搜寻状态，跨天可再次领取少量物资。
- 平安镇作为起始镇落在北洛阳区域，新手镇 NPC、洛阳主线 NPC、镇东敌人与门派 NPC 已分层摆放，避免开局人群全部堆在洛阳中心。

## 6.2 当前剧情与核心角色落地

- 已加入第一批文档核心 NPC：苏梦瑶、陈天行、赵无极、玄机子、花如玉、烈火、蛇王、太极真人、冰魄、逍遥子。
- 已加入主线开端任务：`q_main_luoyang_ashes`、`q_main_shadow_letters`、`q_main_sect_warnings`、`q_main_shadow_watchers`、`q_main_broken_token`、`q_main_wulin_conclave`。
- 已加入七派第一版入门任务线：每派 3 段，覆盖入门练功、门内事务和外出试炼，并可奖励本门武学。
- 这些任务先承担“把玩家带到五城与七派”的作用，后续再扩展选择、分支后果、NPC 好感、门派晋升与暗影司主线。

## 7. 当前美术落地

- `tools/generate_godot_art_assets.py` 生成第一批 Godot 可直接使用的 2D 美术资源：20 张地图瓦片、99 张 NPC 地图 sprite、41 张参考级 NPC archetype、99 张 NPC 对话头像、16 张玩家门派/性别 sprite、45 张角色拆件 PNG、22 张物品图标、41 张武学图标、73 张区域场景背景、8 张 UI 资源；`tools/refine_world_tiles.py` 负责迭代当前运行时使用的手绘瓦片多变体，`tools/generate_world_prop_assets.py` 负责生成 2.5D 地图道具 PNG。
...
- `godot_project/assets/world/stage_layers/` 保存横版局部舞台的生成地面/中景/前景 PNG 分层，当前清河镇已接入 v2 DNF 城镇式同源分层：高细节连续店铺中景、透视砖街地面和木梁/栏杆/货箱近景遮挡层。
