...
- `LocalAreaMap`：承接世界地图进入后的城镇/门派/野外局部地图和商铺内景，负责区域内商铺入口、相邻区域路牌、返回世界入口、局部 NPC 摆放和内景碰撞。
...
- 关键探索链路使用 `res://tests/playtest_smoke.tscn` 和 `res://tests/playtest_flow.tscn` 做自动游玩测试：前者覆盖世界地图、局部地图、商铺内景、NPC 分布、碰撞、相邻区域入口和 2.5D 遮挡层基础检查；后者实例化主场景，自动跑新游戏、进入平安镇、进入商铺、与掌柜对话、打开交易、购买商品、出门、通过相邻区域路牌转场并返回世界地图。
