...
- `tools/generate_godot_art_assets.py` 已按“参考图抠图 + 组件化回退”规则生成第一批可直接使用资源：`99` 个 NPC 地图 sprite、`41` 个参考级 NPC archetype、`99` 个 NPC 对话头像、`16` 个玩家门派/性别 sprite、`45` 个头部/服饰/道具拆件 PNG、`20` 个地图瓦片 PNG、`22` 个物品图标、`41` 个武学图标、`73` 个区域场景背景、`8` 个基础 UI PNG。
- 当前大地图 NPC 优先使用 `godot_project/assets/characters/reference_map_sprites/` 的高精度水墨 Q 版透明 PNG；未覆盖的 NPC 才回退到 `godot_project/assets/characters/generated_map_sprites/` 的统一风格组件化透明 PNG，避免整身 PNG 与临时程序小人混用导致精细度不一致。
...
- 旧 PNG 图集、切片后的单角色 sprite 和 prompt 源已清理，当前保留 `reference_map_sprites/`、`generated_map_sprites/`、`npc/portraits/` 与 `characters/parts/` 作为运行时资源和生成源。
...
- `godot_project/data/skill_icon_assets.json` 负责把 41 个武学 ID 映射到图标。
