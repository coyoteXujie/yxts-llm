...
def generate_tiles() -> dict[str, str]:
    def grass(p: Painter) -> None:
        for i in range(10):
            x = 4 + i * 5
            y = 35 + (i % 3) * 2
            p.line([(x, y), (x + 3, y - 9)], color((0.54, 0.65, 0.32), 110), 1.1)
            p.line([(x + 2, y - 5), (x + 7, y - 8)], color((0.42, 0.58, 0.28), 85), 1.0)
        for cx, cy in [(12, 16), (31, 14), (39, 30)]:
            p.ellipse((cx - 2, cy - 1, cx + 2, cy + 1), color((0.82, 0.74, 0.48), 60))

    def road(p: Painter) -> None:
        p.polygon([(0, 27), (48, 18), (48, 35), (0, 43)], color((0.68, 0.58, 0.40), 155))
        p.line([(0, 15), (48, 8)], color((0.34, 0.27, 0.18), 55), 1)
        p.line([(0, 44), (48, 35)], color((0.22, 0.16, 0.10), 42), 1)
        for x, y, r in noise_points("road_stone", 12):
            p.ellipse((x - r * 1.5, y - r, x + r * 1.5, y + r), color((0.26, 0.20, 0.14), 82), color((0.72, 0.62, 0.44), 35), 1)
        for x in range(5, 48, 11):
            p.line([(x, 24), (x + 10, 22)], color((0.80, 0.70, 0.50), 42), 1)

    def water(p: Painter) -> None:
        p.polygon([(0, 12), (48, 8), (48, 48), (0, 48)], color((0.10, 0.28, 0.40), 78))
        for y in (13, 23, 33, 41):
            p.arc((3, y - 6, 25, y + 6), 190, 350, color((0.72, 0.88, 0.95), 110), 1.4)
            p.arc((20, y - 7, 46, y + 7), 190, 350, color((0.09, 0.20, 0.28), 70), 1.2)
        p.line([(0, 5), (48, 2)], color((0.78, 0.72, 0.48), 35), 1)

    def building(p: Painter) -> None:
        p.rect((6, 17, 42, 42), color((0.38, 0.22, 0.14), 178), color((0.19, 0.10, 0.06), 190), 2)
        p.polygon([(2, 18), (24, 5), (46, 18), (40, 23), (8, 23)], color((0.24, 0.10, 0.07), 210))
        p.line([(5, 21), (43, 21)], color((0.78, 0.46, 0.22), 120), 2)
        for x in (12, 20, 28, 36):
            p.line([(x, 14), (x - 6, 22)], color((0.63, 0.34, 0.17), 135), 1)
        p.rect((18, 27, 30, 42), color((0.14, 0.08, 0.04), 120), color((0.74, 0.52, 0.28), 70), 1)
        p.ellipse((37, 26, 42, 33), color((0.95, 0.46, 0.22), 90))
    ...
    def courtyard(p: Painter) -> None:
        for y in range(7, 46, 9):
            p.line([(3, y), (45, y - 2)], color((0.64, 0.58, 0.42), 78), 1)
        for x in range(8, 46, 12):
            p.line([(x, 4), (x - 4, 44)], color((0.26, 0.22, 0.16), 42), 1)
        p.rect((8, 9, 40, 39), color((0.20, 0.17, 0.12), 34), color((0.74, 0.64, 0.42), 86), 1)
    ...
    def bridge(p: Painter) -> None:
        p.rect((4, 15, 44, 35), color((0.52, 0.31, 0.16), 180), color((0.22, 0.11, 0.05), 145), 1)
        p.arc((3, 10, 45, 42), 185, 355, color((0.80, 0.58, 0.30), 140), 2)
        for x in range(9, 44, 7):
            p.line([(x, 11), (x, 39)], color((0.24, 0.13, 0.06), 150), 1.2)
        p.line([(5, 20), (43, 20)], color((0.86, 0.64, 0.34), 125), 2)
        p.line([(5, 31), (43, 31)], color((0.30, 0.15, 0.07), 95), 1.6)

    def field(p: Painter) -> None:
        for y in (11, 20, 29, 38):
            p.line([(2, y), (46, y - 7)], color((0.74, 0.72, 0.32), 126), 1.5)
            p.line([(2, y + 3), (46, y - 4)], color((0.26, 0.30, 0.12), 58), 1)
        for x in (10, 23, 36, 45):
            p.line([(x, 6), (x - 9, 43)], color((0.22, 0.27, 0.12), 66), 1)

    def mountain(p: Painter) -> None:
        p.polygon([(0, 45), (17, 11), (36, 45)], color((0.24, 0.24, 0.22), 185))
        p.polygon([(13, 25), (17, 11), (23, 27)], color((0.78, 0.76, 0.66), 116))
        p.polygon([(18, 45), (34, 16), (49, 45)], color((0.16, 0.16, 0.15), 132))
        p.line([(17, 11), (28, 45)], color((0.06, 0.06, 0.05), 76), 1.5)
        p.line([(34, 16), (39, 45)], color((0.72, 0.68, 0.56), 45), 1)
    ...
    def shop(p: Painter) -> None:
        p.rect((6, 17, 42, 42), color((0.62, 0.38, 0.22), 152), color((0.25, 0.13, 0.06), 130), 1)
        p.polygon([(3, 18), (24, 7), (45, 18), (39, 23), (9, 23)], color((0.32, 0.12, 0.07), 190))
        p.line([(7, 22), (41, 22)], color((0.94, 0.70, 0.32), 150), 2)
        p.rect((13, 9, 35, 17), color((0.78, 0.54, 0.24), 140), color((0.22, 0.11, 0.05), 90), 1)
        p.rect((19, 28, 30, 42), color((0.12, 0.07, 0.04), 105))
        p.line([(17, 32), (32, 32)], color((0.88, 0.64, 0.32), 70), 1)

    def temple(p: Painter) -> None:
        p.polygon([(3, 18), (24, 5), (45, 18), (39, 23), (9, 23)], color((0.67, 0.45, 0.22), 185))
        p.polygon([(8, 27), (24, 17), (40, 27), (36, 31), (12, 31)], color((0.45, 0.28, 0.14), 150))
        p.rect((12, 23, 36, 41), color((0.32, 0.24, 0.17), 142))
        p.line([(24, 8), (24, 41)], color((0.82, 0.66, 0.36), 80), 1)
        p.ellipse((21, 30, 27, 36), color((0.88, 0.62, 0.25), 65))
    ...
    def bamboo(p: Painter) -> None:
        for x in (9, 18, 28, 38):
            p.line([(x, 6), (x - 4, 43)], color((0.09, 0.26, 0.10), 198), 2)
            for y in (15, 25, 35):
                p.line([(x - 1, y), (x + 8, y - 5)], color((0.36, 0.66, 0.28), 134), 1.6)
                p.line([(x - 2, y + 4), (x - 10, y)], color((0.28, 0.54, 0.24), 112), 1.4)
            p.line([(x - 4, 19), (x + 2, 18)], color((0.78, 0.86, 0.45), 80), 1)
