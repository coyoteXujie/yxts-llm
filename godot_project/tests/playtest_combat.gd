...
func _run() -> void:
	...
	_check(COMBAT_STAGE_SCRIPT.COMBAT_ACTOR_FRAME_ANIMATION_ENABLED, "2.5D 战斗舞台应启用动作帧播放管线")
	_check(GameData.get_combat_actor_frame_paths("player_male_none", "attack").size() >= 3, "默认男主应有三段攻击动作帧")
	_check(GameData.get_combat_actor_frame_paths("player_male_none", "hurt").size() >= 2, "默认男主应有受击动作帧")
	_check(GameData.get_combat_actor_frame_paths("npc_流氓", "attack").size() >= 3, "开局流氓应有三段攻击动作帧")
	_check(GameData.get_combat_actor_frame_paths("npc_流氓头", "down").size() >= 1, "开局流氓头应有倒地动作帧")
	_check(stage.player_frame_textures.has("attack") and stage.player_frame_textures["attack"].size() >= 3, "战斗舞台应加载玩家攻击帧纹理")
	_check(stage.enemy_frame_textures.has("hurt") and stage.enemy_frame_textures["hurt"].size() >= 2, "战斗舞台应加载敌人受击帧纹理")
	var thug_leader_sprite_path := GameData.get_npc_sprite_path("流氓头")
	var thug_leader_texture := GameData.load_texture(thug_leader_sprite_path, true)
	_check(thug_leader_sprite_path.ends_with("thug_leader_stage_v2.png"), "开局流氓头应使用高质量敌人 sprite")
	_check(thug_leader_texture != null and thug_leader_texture.get_size().y >= 1200.0, "2.5D 战斗舞台流氓头不应继续使用低清小图")
	_check(stage.background_texture != null, "2.5D 战斗舞台应加载当前区域背景")
	_check(str(stage.current_region.get("id", "")) == "qinghe", "2.5D 战斗舞台应读取当前区域作为舞台背景语义")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_FOUNDATION_ENABLED, "2.5D 战斗舞台应使用连续横版舞台基底")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_ASSET_LAYERS_ENABLED, "2.5D 战斗舞台应启用独立分层舞台资产")
	_check(GameData.get_combat_stage_layer_path("qinghe", "backdrop").ends_with("qinghe_combat_backdrop_v1.png"), "清河镇战斗舞台应有独立后景资产")
	_check(GameData.get_combat_stage_layer_path("qinghe", "midground").ends_with("qinghe_combat_midground_v1.png"), "清河镇战斗舞台应有独立中景资产")
	_check(GameData.get_combat_stage_layer_path("qinghe", "floor").ends_with("qinghe_combat_floor_v1.png"), "清河镇战斗舞台应有独立地面资产")
	_check(GameData.get_combat_stage_layer_path("qinghe", "foreground").ends_with("qinghe_combat_foreground_v1.png"), "清河镇战斗舞台应有独立前景遮挡资产")
	_check(stage.stage_backdrop_texture != null and stage.stage_backdrop_texture.get_size().x >= 1280.0 and stage.stage_backdrop_texture.get_size().y >= 720.0, "清河镇战斗舞台后景不应低于 1280x720")
	_check(stage.stage_midground_texture != null and stage.stage_floor_texture != null and stage.stage_foreground_texture != null, "清河镇战斗舞台四层资产应全部加载")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_BACKGROUND_TEXTURE_ALPHA <= 0.62, "2.5D 战斗舞台区域背景不应完全压住程序化舞台纵深")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_BACKDROP_ALPHA >= 0.92, "独立战斗舞台后景应成为主视觉而不是弱贴图")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_FOREGROUND_ALPHA >= 0.86, "独立战斗舞台前景遮挡应有足够压场强度")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_ACTOR_MAX_SCALE >= 1.60, "全屏 2.5D 战斗舞台应允许角色随舞台高度放大")
	_check(COMBAT_STAGE_SCRIPT.COMBAT_STAGE_MOTION_MAX_SCALE >= 1.40, "全屏 2.5D 战斗舞台应允许出招位移随舞台高度放大")
	...
	var combat_panel = COMBAT_PANEL_SCRIPT.new()
	combat_panel.size = Vector2(1280, 720)
	add_child(combat_panel)
	_check(COMBAT_PANEL_SCRIPT.IMMERSIVE_STAGE_ENABLED, "战斗 UI 应启用沉浸式全屏舞台布局")
	_check(COMBAT_PANEL_SCRIPT.COMBAT_ACTION_COLUMNS >= 4, "战斗 UI 底部操作区应横向容纳主要招式按钮")
	_check(combat_panel.stage != null and combat_panel.stage.anchor_right == 1.0 and combat_panel.stage.anchor_bottom == 1.0, "战斗 UI 舞台应铺满主视野")
	_check(combat_panel.player_bar != null, "战斗 UI 顶部应显示我方状态条")
	...
	_check(str(player_attack_pose.get("frame_action", "")) == "attack", "玩家攻击姿态应切换到攻击动作帧")
	_check(int(player_attack_pose.get("frame_index", -1)) >= 1, "玩家命中段应使用攻击动作中后段帧")
	...
	_check(str(enemy_hurt_pose.get("frame_action", "")) == "hurt", "敌人受击姿态应切换到受击动作帧")
	stage.update_snapshot({
		"enemy": GameData.get_npc_by_name("流氓"),
