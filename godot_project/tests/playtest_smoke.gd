...
func _run() -> void:
	...
	_check(GameData.get_stage_layer_path("qinghe", "floor").ends_with("qinghe_floor_layer_v1.png"), "清河镇应映射专属横版地面贴图层")
	...
	_check(GameData.get_stage_layer_path("qinghe", "midground").ends_with("qinghe_shopfront_layer_v2.png"), "清河镇应映射专属横版店铺中景层")
	...
	_check(GameData.get_stage_layer_path("qinghe", "foreground").ends_with("qinghe_foreground_layer_v1.png"), "清河镇应映射专属横版前景遮挡层")
	var qinghe_foreground_layer := GameData.load_texture(GameData.get_stage_layer_path("qinghe", "foreground"), true)
	_check(qinghe_foreground_layer != null and qinghe_foreground_layer.get_size().x >= 1700.0 and qinghe_foreground_layer.get_size().y >= 800.0, "清河镇前景遮挡层应具备横版舞台分辨率")
	_check(LOCAL_AREA_SCRIPT.SIDE_VIEW_PAINTED_MIDGROUND_LAYER_ALPHA >= 0.58, "清河镇店铺中景层应有足够可见度")
