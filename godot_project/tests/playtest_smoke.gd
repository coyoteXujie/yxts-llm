...
func _run() -> void:
	...
	var blade_effects: Dictionary = GameData.get_item("item_blade").get("effects", {})
	var worn_blade_attack_bonus := int(round(float(blade_effects.get("attack", 0)) * GameState.get_equipment_durability_effect_factor("item_blade")))
	_check(worn_blade_attack_bonus < int(blade_effects.get("attack", 0)) and int(GameState.player.get("attack", 0)) == base_attack + worn_blade_attack_bonus, "磨损装备应按耐久阶段降低当前攻防加成")
	...
	if blade_index >= 0:
		...
		_check(inventory_panel.details.text.contains("已装备：武器") and inventory_panel.details.text.contains("当前武器：雁翎刀") and inventory_panel.details.text.contains("当前加成：攻击 %+d" % worn_blade_attack_bonus) and inventory_panel.details.text.contains("耐久：72/100") and inventory_panel.details.text.contains("加成85%") and inventory_panel.details.text.contains("铁匠修理"), "背包装备详情应显示当前武器槽、有效加成和耐久维护信息")
	var sword_index: int = inventory_panel.item_ids.find("item_sword")
	if sword_index >= 0:
		inventory_panel._select_item(sword_index)
		_check(inventory_panel.details.text.contains("当前武器：雁翎刀") and inventory_panel.details.text.contains("更换变化：无"), "背包详情应按当前耐久后的有效加成计算换装变化")
	...
	_check(not LOCAL_AREA_SCRIPT.SHOP_INTERIOR_TEXTURE_PROCEDURAL_GOODS_ENABLED, "有整屏 v2 内景时不应再叠粗糙程序化货物")
	_check(LOCAL_AREA_SCRIPT.SHOP_INTERIOR_TEXTURE_SERVICE_ZONE_ALPHA <= 0.22 and LOCAL_AREA_SCRIPT.SHOP_INTERIOR_TEXTURE_FOREGROUND_FRAME_ALPHA <= 0.26, "整屏内景上方只应保留轻量站位和边缘压暗")
	...
	_check(LOCAL_AREA_SCRIPT.SHOP_INTERIOR_COUNTER_ALPHA >= 0.86, "无整屏 PNG 时商铺内景柜台应作为兜底主视觉层")
	_check(LOCAL_AREA_SCRIPT.SHOP_INTERIOR_SHELF_ALPHA >= 0.76, "无整屏 PNG 时商铺内景应有可见货架层")
	_check(LOCAL_AREA_SCRIPT.SHOP_INTERIOR_THEME_PROP_ALPHA >= 0.82, "无整屏 PNG 时商铺内景应按店铺类型绘制主题道具")
	...
	if not shop_portal.is_empty():
		...
		_check(local_area.prop_nodes.is_empty(), "有整屏商铺内景时不应再生成店内货架/摊位道具节点")
		...
		if local_area.npc_nodes.size() > 0:
			...
			if market_clues.size() > 0:
				...
				_check(market_journal_text.contains("需带：鲜鱼 x3") and market_journal_text.contains(str(latest_market_clue.get("target_region_name", ""))), "任务江湖页应显示货价线索需携带的特产数量和交割目的地")
				quest_panel.free()
			...
			if not market_target_region_id.is_empty():
				...
				if not market_clue_portal.is_empty():
					...
					_check(str(market_clue_portal.get("delivery_item_id", "")) == "item_fish" and int(market_clue_portal.get("delivery_count", 0)) == 3 and int(market_clue_portal.get("delivery_bonus_money", 0)) >= 10, "货价线索入口应按货值要求多件特产并配置交割收益")
					_check(int(market_clue_portal.get("reward_money", 0)) >= 8 and int(market_clue_portal.get("reward_exp", 0)) >= 8, "货价线索入口应有询价基础奖励")
					var inventory_before_market_delivery: Dictionary = GameState.inventory.duplicate(true)
					GameState.inventory.erase("item_fish")
					var market_money_before_missing := int(GameState.player.get("money", 0))
					...
					_check(not GameState.is_adventure_clue_resolved("market_linan_item_fish") and int(GameState.player.get("money", 0)) == market_money_before_missing, "空手询价不应完成货价线索或发放奖励")
					GameState.add_item("item_fish", 1)
					main_controller.call("_inspect_adventure_clue", market_clue_portal)
					_check(not GameState.is_adventure_clue_resolved("market_linan_item_fish") and int(GameState.inventory.get("item_fish", 0)) == 1, "货物数量不足时不应完成货价交割")
					GameState.add_item("item_fish", 2)
					...
					_check(int(GameState.inventory.get("item_fish", 0)) == 0, "带足数量后交割应消耗对应特产")
					...
					if not trade_records.is_empty():
						...
						_check(str(latest_trade_record.get("item_id", "")) == "item_fish" and int(latest_trade_record.get("count", 0)) == 3 and int(latest_trade_record.get("profit", 0)) == int(market_clue_portal.get("delivery_bonus_money", 0)) + int(trade_route_plan.get("trade_risk_bonus", 0)) and str(latest_trade_record.get("risk_label", "")) == str(trade_route_plan.get("risk_label", "")), "跑商履历应记录交割物品数量、收益和路线风险")
