...
func _run() -> void:
	...
	if not qinghe.is_empty():
		...
		if not hidden_clue_portal.is_empty():
			...
			await _frames(1)
			_check(main.quest_panel.rumor_text.text.contains("奇遇线索") and main.quest_panel.rumor_text.text.contains(str(hidden_clue_portal.get("label", ""))), "任务日志江湖页应展示追查到的奇遇线索")
			_check(main.quest_panel.quest_text.text.contains("奇遇线索") and main.quest_panel.quest_text.text.contains(str(hidden_clue_portal.get("label", ""))), "任务页应把奇遇线索作为后续行动记录")
