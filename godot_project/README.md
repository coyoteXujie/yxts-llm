...
`playtest_flow` 会实例化主场景，自动跑新游戏、进入平安镇、进入商铺、与掌柜对话、打开交易、购买商品、出门、通过相邻区域路牌转场，并返回世界地图。headless 渲染使用 Godot 的 dummy renderer，不能取真实画面截图；在非 headless 运行时会尝试保存 `user://playtest_flow.png`。
...
- WASD / 方向键：移动。
- T / Enter：与附近 NPC 交谈。
- E：进入当前城镇/门派/野外局部地图；在局部地图中进入商铺、出门、返回世界地图或通过路牌前往相邻区域。
- F：挑战附近敌人。
- B：背包，使用药品或装备武器/防具。
- J：任务日志。
- K：修炼面板。
