...
`playtest_flow` 会实例化主场景，自动跑新游戏、进入平安镇、互动探索地标、进入商铺、与掌柜对话、打开交易、购买商品、出门、通过相邻区域路牌转场，并返回世界地图。headless 渲染使用 Godot 的 dummy renderer，不能取真实画面截图；在非 headless 运行时会尝试保存 `user://playtest_flow.png`。
...
- WASD / 方向键：移动。
- T / Enter：与附近 NPC 交谈。
- E：进入当前城镇/门派/野外局部地图；在局部地图中进入商铺、探索地标、出门、返回世界地图或通过路牌前往相邻区域。
- F：挑战附近敌人。
- B：背包，使用药品或装备武器/防具。
- J：任务日志。
- K：修炼面板。
...
- 当前有平安镇任务链、镇东敌人、采花大盗、阶段通关试炼，“洛阳旧火/暗影书信/七派风声/暗影眼线/断令归卷/武林夜议”的主线开端，以及七派各 3 段入门任务线。
...
- 已加入第一版转入转出流程：世界地图可进入城池、城镇、门派、野外的局部地图；局部地图内可进入客栈、药铺、铁匠铺、布庄、市集、茶肆等商铺内景，商铺掌柜沿用 NPC 交易/住店系统；局部地图会按区域关系生成相邻区域路牌，可从镇子直接走向附近城池、野外或门派。
- 局部地图已加入第一批可互动地标和资源点：五大城池有旧宅、榜亭、码头、药市、战鼓台等特色点；城镇、门派和野外会按地形生成告示、练武场、山洞、药坡、浅滩、古碑等入口，首次探索会弹出发现面板并获得少量物品、银两或阅历。
...
- 发现面板用于承载局部地图地标、资源点和后续奇遇事件的标题、正文和奖励反馈。
